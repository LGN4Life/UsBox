function attention_lfp = batch_attention_lfp(data_set,varargin)
...
for file_index =LoopVar.start_index:LoopVar.end_index
    ...
    if current_excel_line.Exclude == 0 & contains(current_excel_line.FileName,'atn') & current_excel_line.ContinuousChannel>0
        ...
        if str2num(data.Parameters.Stimulus.TemporalFrequency)>.1
            ...
            attention_lfp.alpha_change_rate(cn,:) = [percent_change.raw nanmean(percent_change.shuffle)];
            ...
            attention_lfp.spectrogram_time = spectrogram.x;
...
%             figure(11),plot(squeeze(nanmean(nanmean(spectrogram.y_bs),3)))
            ...
            attention_lfp.spectrogram(cn,:,:) = squeeze(nanmean(spectrogram.y));
            attention_lfp.spectrogram_norm(cn,:,:) = squeeze(nanmean(spectrogram.y_norm));
            
            attention_lfp.spectrogram_away(cn,:,:) = squeeze(nanmean(spectrogram.y(away_trials,:,:)));
            attention_lfp.spectrogram_away_norm(cn,:,:) = squeeze(nanmean(spectrogram.y_norm(away_trials,:,:)));
            
            attention_lfp.spectrogram_toward(cn,:,:) = squeeze(nanmean(spectrogram.y(toward_trials,:,:)));
            attention_lfp.spectrogram_toward_norm(cn,:,:) = squeeze(nanmean(spectrogram.y_norm(toward_trials,:,:)));
            ...
            attention_lfp.alpha_stim_time = stim_locked.x;
            attention_lfp.alpha_stim_locked(cn,:) = nanmean(stim_locked.y,2)/nanmean(alpha_power.Y);;
            attention_lfp.alpha_stim_locked_away(cn,:) = nanmean(stim_locked.y(:,away_trials),2)/nanmean(alpha_power.Y);;
            attention_lfp.alpha_stim_locked_toward(cn,:) = nanmean(stim_locked.y(:,toward_trials),2)/nanmean(alpha_power.Y);;
            ...
            attention_lfp.alpha_stim_random(cn,:) = nanmean(stim_locked.y,2)/nanmean(alpha_power.Y);
...
%             hold off
            ...
            attention_lfp.away_auto_corr(cn,:) = xcorr(B(1,:),'normalized',1000);
            attention_lfp.away_scramble_corr(cn,:) = xcorr(B_scramble(1,:),'normalized',1000);
        
            attention_lfp.stim_auto_corr(cn,:) = xcorr(B(stim_band,:),'normalized',1000);
            attention_lfp.stim_scramble_corr(cn,:) = xcorr(B_scramble(stim_band,:),'normalized',1000);
            
            attention_lfp.alpha_auto_corr(cn,:) = xcorr(B(alpha_peak_id,:),'normalized',1000);
            attention_lfp.alpha_scramble_corr(cn,:) = xcorr(B_scramble(alpha_peak_id,:),'normalized',1000);
            attention_lfp.gamma_auto_corr(cn,:) = xcorr(B(gamma_peak_id,:),'normalized',1000);
            attention_lfp.gamma_scramble_corr(cn,:) = xcorr(B_scramble(gamma_peak_id,:),'normalized',1000);
            
            attention_lfp.toward_auto_corr(cn,:) = xcorr(B(end,:),'normalized',1000);
            attention_lfp.toward_scramble_corr(cn,:) = xcorr(B_scramble(end,:),'normalized',1000);
            
            
            attention_lfp.away_ft(cn,:) = LFP.custom_fft(attention_lfp.away_auto_corr(cn,:)',f2,fs);
            attention_lfp.away_scramble_ft(cn,:) = LFP.custom_fft(attention_lfp.away_scramble_corr(cn,:)',f2,fs);
            
            attention_lfp.stim_ft(cn,:) = LFP.custom_fft(attention_lfp.stim_auto_corr(cn,:)',f2,fs);
            attention_lfp.stim_scramble_ft(cn,:) = LFP.custom_fft(attention_lfp.stim_scramble_corr(cn,:)',f2,fs);
            
            attention_lfp.alpha_ft(cn,:) = LFP.custom_fft(attention_lfp.alpha_auto_corr(cn,:)',f2,fs);
            attention_lfp.alpha_scramble_ft(cn,:) = LFP.custom_fft(attention_lfp.alpha_scramble_corr(cn,:)',f2,fs);
            attention_lfp.gamma_ft(cn,:) = LFP.custom_fft(attention_lfp.gamma_auto_corr(cn,:)',f2,fs);
            attention_lfp.gamma_scramble_ft(cn,:) = LFP.custom_fft(attention_lfp.gamma_scramble_corr(cn,:)',f2,fs);
            
            attention_lfp.toward_ft(cn,:) = LFP.custom_fft(attention_lfp.toward_auto_corr(cn,:)',f2,fs);
            attention_lfp.toward_scramble_ft(cn,:) = LFP.custom_fft(attention_lfp.toward_scramble_corr(cn,:)',f2,fs);
...
figure(1),plot(attention_lfp.f2,nanmean(abs(attention_lfp.alpha_ft) - abs(attention_lfp.alpha_scramble_ft))./nanmean(abs(attention_lfp.alpha_ft)))
hold on
figure(1),plot(attention_lfp.f2,nanmean(abs(attention_lfp.gamma_ft) - abs(attention_lfp.gamma_scramble_ft))./nanmean(abs(attention_lfp.gamma_ft)))
...
figure(2),plot(attention_lfp.alpha_stim_time,nanmean(attention_lfp.alpha_stim_locked))
hold on
figure(2),plot(attention_lfp.alpha_stim_time,nanmean(attention_lfp.alpha_stim_locked_away))
figure(2),plot(attention_lfp.alpha_stim_time,nanmean(attention_lfp.alpha_stim_locked_toward))
figure(2),plot(attention_lfp.alpha_stim_time,nanmean(attention_lfp.alpha_stim_random))
hold off
legend('all','away','toward','random')
xlabel('time')
ylabel('amp')

figure(4),subplot(1,3,1),imagesc(squeeze(nanmean(attention_lfp.spectrogram_norm))), axis square
figure(4),subplot(1,3,2),imagesc(squeeze(nanmean(attention_lfp.spectrogram_away_norm))), axis square
figure(4),subplot(1,3,3),imagesc(squeeze(nanmean(attention_lfp.spectrogram_toward_norm))), axis square
figure(5),imagesc((squeeze(nanmean(attention_lfp.spectrogram_toward_norm))-...
    squeeze(nanmean(attention_lfp.spectrogram_away_norm)))./(squeeze(nanmean(attention_lfp.spectrogram_toward_norm)))), axis square
colorbar
end
