function contrast_hrat(data_set,varargin)
...
for file_index = start_index:end_index
    ...
    if data_set.SpikeChannel(file_index)>0 & data_set.Exclude(file_index)==0 & contains(data_set.FileName{file_index},'con')
        ...
        if ~isempty(CompletedTrials)
            ...
            if length(tuning_data.IV)>3
                ...
                [fit_data.Param,fit_data.New_X,fit_data.New_Y] = HRat_fit_call(contrast,response);
                temp = find(fit_data.New_Y>.5);
                
                if isempty(temp)
                    fit_data.c50 =nan;
                else
                    fit_data.c50 = fit_data.New_X(temp(1));
                end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if exist([current_folder 'contrast.mat'],'file')
            save([current_folder 'contrast.mat'],'tuning_data','fit_data', '-append')
        else
            save([current_folder 'contrast.mat'],'tuning_data','fit_data')
        end
