function data = LoadParFileInfo(data)
data.Parameters.TargetList =[];

h = fopen(data.Parameters.FileNames.ParFile);
    ...
    if contains(current_heading, 'Targets')
        ...
        if ~contains(rawParameters{index},' SUCCESS FAIL')
            ...
            if ~isempty(current_target)
                ...
                data.Parameters.TargetList= cat(1,data.Parameters.TargetList, current_target);
        ...
        if length(varBreakIndex)==1
            ...
            data.Parameters.(current_heading).(varName) = varValue;
